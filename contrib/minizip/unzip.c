...
/* OpenHarmony extensions: preserve the existing FILE ownership contract. */
...
extern int ZEXPORT unzLocateFile2 (unzFile file, const char *szFileName, int iCaseSensitivity)
{
...

    {
        ...
        if (err == UNZ_OK && unzStringFileNameCompare(szCurrentFileName, szFileName, iCaseSensitivity) == 0) {
            return UNZ_OK;
        }
    }
